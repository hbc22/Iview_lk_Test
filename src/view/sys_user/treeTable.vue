<template>
    <div :style="{width:tableWidth}" class='autoTbale'>
        <table class="table table-bordered" id='hl-tree-table'>
            <thead>
                <tr>
                    <th v-for="(index, column) in cloneColumns" :key="index">
            ...
            <tbody>
                <tr v-for="(index, item) in initItems" :key="index" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column, i) in columns" :key="i" :style=tdWidth(column.width)>
                        ...
                        <div v-if="column.type === 'action'">
                            <i-button :type="action.type" size="small" @click="RowClick(item,index,$event,action.text)" v-for='(action, j) in column.actions' :key="j">{{action.text}}</i-button>
                        </div>
